...
class OFInterface {
  public:
    ...
    void install_default_meter(fluid_base::OFConnection* of_conn);
/*
 * MeteringTable
 * ##################################################################
 * install_meter_mod - installing the metering table rules
 ...
 */
    void install_meter_mod(fluid_msg::of13::PacketIn &pi, fluid_base::OFConnection* ofconn, uint64_t src, uint64_t dst, uint16_t out_port, uint32_t meter_id, uint16_t type, uint16_t len, uint32_t rate, uint32_t burst_size);
 ...
 */
    void get_meter_stats(fluid_base::OFConnection* of_conn, uint32_t xid, uint32_t meter_id);
/*
 * ##################################################################
 */
